...
class autoEDA:
    ...
    def select_features(self, target_column, threshold=0.0):
        ...
        for c in low_variance_cols :
            if  c!=target_column:
                df = df.drop(column=c, inplace=True)
        return df
    ...
    def generate_dashboard(self, target_column):
        ...
        for column in df.columns:
            if (df[column].dtype == "int64" or df[column].dtype == "float64") and column!=target_column:
                ...
                generate_scatter_plot(df, column, target_column)  # Replace "target_column" with the appropriate target column
                #generate_interactive_plot(df, column, target_column, color_column=None)  # Replace "target_column" with the appropriate target column
